...
// Data Model for Logged Races / Active Queue
class RaceLogEntry {
  final String id;
  final String event;
...
class _DashboardPageState extends State<DashboardPage> {
  ...
  // Mental State & Reaction Readiness Variables
  ...
  final List<String> _mentalStatesList = [
    "Focused",
    "Anxious",
    "Calm",
    "Fatigued",
  ...
  // Dynamic Split Calculator based on exact event distance
  ...
  void _startRaceSession(UserProfile profile) {
    ...
    String? activeEvent = _selectedActiveEvent;
    if ((activeEvent == null || activeEvent.isEmpty) &&
        profile.events.isNotEmpty) {
      activeEvent = profile.events.first.eventName;
    }
    ...
    setState(() {
      _activeRaceQueue.insert(0, newEntry);
      _reactionTimeMs = null;
      _reactionStatus =
          "Tap 'Start Reaction Test' to evaluate mental readiness.";
    });
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
          content: Text(
              "Race session started for $activeEvent! Readiness: ${calculatedReadiness.toStringAsFixed(1)}%")),
    );
  }
  ...
  void _showResumeRaceDialog(RaceLogEntry entry) {
    ...
    _splitControllers = List.generate(splitCount, (index) {
      String existingVal =
          (index < entry.splits.length) ? entry.splits[index] : '';
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        title: Text("Resume Race: ${entry.event}",
            ...
            onPressed: () {
              ...
              if (finalT.isNotEmpty) {
                setState(() {
                  ...
                  _activeRaceQueue.removeWhere((e) => e.id == entry.id);
                  _completedRaceHistory.insert(0, entry);
                });
            ...
            children: [
              Text(label,
                  style: const TextStyle(
                      ...
                      fontSize: 13,
                      fontWeight: FontWeight.bold)),
              Text("Rating: ${value.round()}/${max.toInt()}",
  ...
  @override
  Widget build(BuildContext context) {
    // Reload profile data from storage to guarantee fresh state after returning from Settings
    final profile = UserStorage.profile;
    final eventsList = profile?.events ?? [];

    if (eventsList.isNotEmpty) {
      bool eventExists =
          eventsList.any((e) => e.eventName == _selectedActiveEvent);
      if (_selectedActiveEvent == null || !eventExists) {
        _selectedActiveEvent = eventsList.first.eventName;
      }
    } else {
  ...
  Widget _buildRaceStartTab(UserProfile? profile) {
    final eventsList = profile?.events ?? [];
    ...
    return Scrollbar(
      ...
      child: SingleChildScrollView(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            const Text("Mental Readiness & Race Start",
                style: TextStyle(
                    color: Colors.cyanAccent,
                    fontSize: 22,
                    fontWeight: FontWeight.bold)),
            const SizedBox(height: 16),
            if (_activeRaceQueue.isNotEmpty) ...[
              Container(
                padding: const EdgeInsets.all(16),
                decoration: BoxDecoration(
                  ...
                  children: [
                    const Row(
                      children: [
                        ...
                        SizedBox(width: 8),
                        Text("Active Race Queue (Pending Results)",
                            style: TextStyle(
                                color: Colors.greenAccent,
                                fontWeight: FontWeight.bold,
                                fontSize: 16)),
                    ...
                    ListView.builder(
                      shrinkWrap: true,
                      physics: const NeverScrollableScrollPhysics(),
                      itemCount: _activeRaceQueue.length,
                      itemBuilder: (context, index) {
                        final item = _activeRaceQueue[index];
                        return Container(
            ...
            Container(
              ...
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  const Text("1. Select Event for Upcoming Race",
                      style: TextStyle(
                          color: Colors.cyanAccent,
                          fontWeight: FontWeight.bold,
                          fontSize: 16)),
                  ...
                  DropdownButtonFormField<String>(
                    initialValue: _selectedActiveEvent,
                    ...
                    items: eventsList
                        .map((e) => DropdownMenuItem(
                            value: e.eventName,
                            child: Text(
                                "${e.eventName} (PB: ${e.personalBest.isEmpty ? 'N/A' : e.personalBest})")))
                        .toList(),
                    onChanged: (val) =>
                        setState(() => _selectedActiveEvent = val),
                  ),
                ],
              ),
            ),
            ...
            Container(
              ...
              child: Column(
                ...
                children: [
                  const Text(
                      "2. Mental States & Individual Ratings (Select & Rate)",
                      style: TextStyle(
                          color: Colors.cyanAccent,
                          fontWeight: FontWeight.bold,
                          fontSize: 16)),
                  ...
                  Wrap(
                    spacing: 8.0,
                    children: _mentalStatesList.map((state) {
                      final isSelected = _selectedMentalStates.contains(state);
                      return FilterChip(
                        label: Text(state),
                        selected: isSelected,
                        ...
                        onSelected: (selected) {
                          setState(() {
                            if (selected) {
                              ...
                              _mentalStateValues.putIfAbsent(state, () => 7.0);
                            } else {
                  ...
                  const Text("Overall Energy Level (1 - 10)",
            ...
            Container(
              ...
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  const Text("3. Capture Neuro Score (Required Reaction Test)",
                      style: TextStyle(
                          color: Colors.amberAccent,
                          fontWeight: FontWeight.bold,
                          fontSize: 16)),
            ...
            SizedBox(
              ...
              child: ElevatedButton.icon(
                onPressed:
                    profile != null ? () => _startRaceSession(profile) : null,
                style: ElevatedButton.styleFrom(
                    backgroundColor: Colors.cyanAccent,
                    foregroundColor: Colors.black),
                icon: const Icon(Icons.play_arrow),
                label: const Text("Start Race Session (Queue Event)",
                    style:
    ...
    return Scrollbar(
      ...
      child: SingleChildScrollView(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            const Text("Progress Tracker & Goal Delta",
                style: TextStyle(
                    color: Colors.purpleAccent,
                    fontSize: 22,
                    fontWeight: FontWeight.bold)),
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Row(
                      mainAxisAlignment: MainAxisAlignment.spaceBetween,
                      children: [
                        const Text("Race Log History",
                            style: TextStyle(
                                color: Colors.purpleAccent,
                                fontWeight: FontWeight.bold,
                                fontSize: 14)),
                        Text("${evHistory.length} entries recorded",
                            style: const TextStyle(
                                color: Colors.white60, fontSize: 12)),
                      ],
                    ),
    ...
    return Scrollbar(
      ...
      child: SingleChildScrollView(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            const Text("Race Predictor & Split Forecasts",
                style: TextStyle(
                    color: Colors.amberAccent,
                    fontSize: 22,
                    fontWeight: FontWeight.bold)),
